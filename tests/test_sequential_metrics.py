...
def _msg(agent, content, turn, duration=None, token_count=None,
         tool_calls=None, ts=None):
    return AgentMessage(
        agent_name=agent,
        content=content,
        turn_number=turn,
...
class TestEmptyMessages:
    def test_empty_returns_defaults(self):
        result = compute_sequential_metrics(
            messages=[], stage_order=["a", "b"], pipeline_template="linear",
        )
...
class TestPerPromptBasic:
    def _linear_msgs(self):
        ...
        return [
            ...
            _msg("executor", "exec output", 2, duration=2.5, token_count=250,
                 tool_calls=[_tc("calculator_tool")], ts=t + 1.0),
    ...
    def test_stage_count(self):
        ...
        result = compute_sequential_metrics(
            msgs, ["planner", "executor", "reviewer"], "linear",
        )
...
class TestToolViolations:
    def test_no_violations(self):
        msgs = [
            _msg("planner", "plan", 1),
            _msg("executor", "exec", 2,
                 tool_calls=[_tc("calculator_tool")]),
        ]
        result = compute_sequential_metrics(
            msgs, ["planner", "executor"], "linear",
            stage_tool_sets={
                "planner": [],
    ...
    def test_mixed_violations(self):
        msgs = [
            _msg("planner", "plan", 1,
                 tool_calls=[_tc("calculator_tool")]),
            _msg("executor", "exec", 2,
                 tool_calls=[_tc("calculator_tool"), _tc("echo_tool")]),
        ]
        result = compute_sequential_metrics(
            msgs, ["planner", "executor"], "linear",
            stage_tool_sets={
        ...
        assert result["tool_restriction_violations"] == 1
        assert result["stage_independence_score"] == pytest.approx(
            1.0 - 1 / 3, abs=0.01
        )
...
class TestInterfaceMetrics:
    def test_all_pass(self):
        ...
        m = compute_sequential_metrics(
            msgs, ["a", "b"], interface_results=results,
        )
...
class TestCrossPromptMetrics:
    ...
    def test_ambidexterity_nonzero(self):
        runs = [
            {"success": True, "metrics": {
                "per_stage_tool_utilization": {"a": 0.0, "b": 1.0}
            }},
            {"success": True, "metrics": {
                "per_stage_tool_utilization": {"a": 0.5, "b": 0.5}
            }},
        ]
        result = compute_cross_prompt_metrics(runs)
    ...
    def test_ambidexterity_zero_uniform(self):
        runs = [
            {"success": True, "metrics": {
                "per_stage_tool_utilization": {"a": 0.5}
            }},
            {"success": True, "metrics": {
                "per_stage_tool_utilization": {"a": 0.5}
            }},
        ...
        runs_vmodel = [
            {
                "success": False,
                "metrics": {
                    "per_stage_tokens": {"r": 80, "s": 90, "d": 100,
                                          "i": 300, "v": 60},
                    "per_stage_duration": {"r": 0.5, "s": 0.8, "d": 1.0,
                                            "i": 4.0, "v": 0.3},
